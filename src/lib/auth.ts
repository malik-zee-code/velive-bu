...
export const setUser = (user: User) => {
  if (typeof window !== "undefined") {
    localStorage.setItem(
      ...
      JSON.stringify({
        ...user,
        roles: user.roles.map(({ role }: any) => role.toLowerCase()),
      })
    );
  }
};
...
export const refreshAccessToken = async (): Promise<boolean> => {
  ...
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:4000"}/api/users/refresh-token`,
      {
        method: "POST",
        headers: {
    ...
    if (data.data?.accessToken && data.data?.refreshToken) {
      ...
      console.log(
        ...
        oldRefreshToken.substring(0, 20) !== data.data.refreshToken.substring(0, 20)
      );
