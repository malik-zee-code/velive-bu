...
const ADD_SETTING = gql`
  mutation AddSetting($title: String!, $value: String!) {
    insert_settings_one(object: {title: $title, value: $value}) {
      id
      title
      value
    }
...
const UPDATE_SETTING = gql`
  mutation UpdateSetting($id: bigint!, $title: String!, $value: String!) {
    update_settings_by_pk(pk_columns: {id: $id}, _set: {title: $title, value: $value}) {
      id
      title
      value
    }
...
const DELETE_SETTING = gql`
  mutation DeleteSetting($id: bigint!) {
...
const SettingsPage = () => {
  ...
  const [addSetting, { loading: addLoading }] = useMutation(ADD_SETTING, {
    onCompleted: () => {
        refetch();
  ...
  const [updateSetting, { loading: updateLoading }] = useMutation(UPDATE_SETTING, {
     onCompleted: () => {
        refetch();
  ...
  const onSubmit = (values: z.infer<typeof formSchema>) => {
    if (editingSetting) {
        updateSetting({ variables: { id: parseInt(editingSetting.id, 10), title: values.title, value: values.value } });
    } else {
        addSetting({ variables: values });
    }
  };
  
  const handleDelete = (id: string) => {
    deleteSetting({ variables: { id: parseInt(id, 10) } });
  };
